...
export class ServerService {
  ...
  getArticlesElectricite(){
    return this.http.get('http://localhost:8080/articles/Electricite');
  }
  ...
  handleAddClient(form: NgForm, Articles: any){
    this.addClient(form.value['nom'],form.value['surnom'],form.value['numero']).subscribe((id: any)=> {
      ...
      this.handleAddArticleAchete(Articles).subscribe();
      this.updateTotalClient(id[0].id,this.totalCLient).subscribe();
    }); 
  }
